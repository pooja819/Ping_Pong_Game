...
let score, maxScore, movement, rod, ballSpeedX = 2,
    ballSpeedY = 2;
...
function storeWin(rod, score) {
    ...
    alert(rod + " wins with a score " + (score * 100) + " .Max Score is " + (maxScore * 100));
    ...
    if (event.code == "Enter") {
        if (!gameOn) {
            ...
            movement = setInterval(function() {
                ...
                if ((ballX + ballDai) > windowWidth || (ballX < 0)) {

                    ballSpeedX = -1 * ballSpeedX; //reverse the direction
                }

                let ballPos = ballX + ballDai / 2;
                ...
                if (ballY <= rod1Height) {
                    ...
                    // either its catching or leaving

                    if ((ballPos < rod1X) || (ballPos > (rod1X + rod1Width))) {
                        storeWin(rod2Name, score);
                    }

                } else if ((ballY + ballDai) >= (windowHeight - rod2Height)) {
